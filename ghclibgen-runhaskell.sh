...
flavors=("ghc-8.8.1")
resolvers=("nightly-2020-01-19" "lts-14.27" "lts-12.25") # ghc-8.8.1, ghc-8.6.5, ghc-8.4.4
for f in "${flavors[@]}"; do
...
flavors=("ghc-8.8.4")
resolvers=("lts-16.31" "lts-14.27" "lts-12.25") # ghc-8.8.4 ghc-8.6.5 ghc-8.4.4
for f in "${flavors[@]}"; do
    for r in "${resolvers[@]}"; do
        echo "-- "
        stack runhaskell CI.hs --stack-yaml stack.yaml --resolver "$r" --package extra --package optparse-applicative -- --ghc-flavor "$f" --stack-yaml stack.yaml --resolver "$r"
...
flavors=("ghc-9.4.5")
resolvers=("nightly-2023-05-07" "lts-20.20" "lts-19.20") # ghc-9.4.5, ghc-9.2.7, ghc-9.0.2
for f in "${flavors[@]}"; do
    for r in "${resolvers[@]}"; do
        echo "-- "
        stack runhaskell CI.hs --stack-yaml stack.yaml --resolver "$r" --package extra --package optparse-applicative -- --ghc-flavor "$f" --stack-yaml stack.yaml --resolver "$r" --no-checkout
